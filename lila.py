...
class GetZAxisLabelExample(ThreeDScene):
    ...
    def construct(self):
        ...
        labx = junction1_ax.get_x_axis_label(Tex("Time").scale(0.7))
        labx.shift(2*LEFT)
        labx.shift(0.7*DOWN)
        laby = junction1_ax.get_y_axis_label(Tex("Flow").scale(0.7))
        laby.shift(1*LEFT)
        laby.shift(0.1*DOWN)
        line1 = junction1_ax.plot(lambda t: np.cos(t) + 0.5 * np.cos(9 * t) + (1 / 7) * np.cos(12 * t) + 2, color=BLUE,)
        junction1_group = VGroup(junction1_ax, line1, labx, laby)
        ...
        labx = junction2_ax.get_x_axis_label(Tex("Time").scale(0.7))
        labx.shift(2*LEFT)
        labx.shift(0.7*DOWN)
        laby = junction2_ax.get_y_axis_label(Tex("Pressure").scale(0.7))
        laby.shift(1.2*LEFT)
        laby.shift(1.6*DOWN)
        line2 = junction2_ax.plot(lambda t: np.cos(t) + 0.5 * np.cos(7 * t) + (1 / 7) * np.cos(14 * t) + 2, color=RED,)
        junction2_group = VGroup(junction2_ax, line2, labx, laby)
        ...
        labx = junction3_ax.get_x_axis_label(Tex("Time").scale(0.7))
        labx.shift(2*LEFT)
        labx.shift(1.6*DOWN)
        laby = junction3_ax.get_y_axis_label(Tex("Pressure").scale(0.7))
        laby.shift(1.2*LEFT)
        laby.shift(1.6*DOWN)
        line3 = junction3_ax.plot(lambda t: np.cos(t) + 0.5 * np.cos(3 * t) + (1 / 7) * np.cos(20 * t) + 2, color=RED,)
        junction3_group = VGroup(junction3_ax, line3, labx, laby)
        ...
        # junction3_group.set_x(0)
        # junction3_group.set_y(0)
        self.wait()

        text = Text('Bernoulli', font_size=16).scale(3).set_y(5)
        tex = Tex(r"$P_i=k^0_{ji}+P_{j}k^i_{ji}+\sum_{d\in D_{IR}}k^d_{ji}Q^2_d$", font_size=16).scale(3).set_y(4)
        self.add(text)
        self.add(tex)
        self.wait()


        main = ThreeDAxes(x_range=(0, 4, 1), 
                          x_length=3, 
                          y_range=(0,4,1), 
                          y_length=3, 
                          z_range=(0,4,1), 
        ...
        main.rotate(45, [1,0,0])
        labels = main.get_axis_labels(Tex("Pressure - $P_i$").scale(0.7), Tex("Flow - $Q_d$").scale(0.7), Tex("Pressure - $P_j$").scale(0.7))
        # line3 = junction3_ax.plot(lambda t: np.cos(t) + 0.5 * np.cos(3 * t) + (1 / 7) * np.cos(20 * t) + 2, color=RED,)
        main_group = VGroup(main, labels)
